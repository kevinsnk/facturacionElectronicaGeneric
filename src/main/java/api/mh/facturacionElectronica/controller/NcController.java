...
import api.mh.facturacionElectronica.ex.model.nc.CuerpoDocumentoDTE;
import api.mh.facturacionElectronica.ex.model.nc.DTE;
import api.mh.facturacionElectronica.ex.model.nc.EmisorDTE;
import api.mh.facturacionElectronica.ex.model.nc.ReceptorDTE;
import api.mh.facturacionElectronica.ex.model.nc.ResumenDTE;
import api.mh.facturacionElectronica.ex.model.nc.VentaTerceroDTE;
import api.mh.facturacionElectronica.jdbc.DteJDBC;
import api.mh.facturacionElectronica.model.FirmardocumentoNCRequest;
...
public class NcController extends AbstractController {

	private static final Logger logger = LogManager.getLogger(NcController.class);
	...
	public RecepcionDTEResponse procesarDTE(Object entity, Object token, Boolean contingencia) {
		...
		FirmardocumentoNCRequest firmardocumentoRequest = new FirmardocumentoNCRequest();
	...
	@Override
	public FirmardocumentoNCRequest obtenerDTE(Object entity, Boolean contingencia) {
		FelControl felControl = (FelControl) entity;
		FirmardocumentoNCRequest firmardocumentoRequest = new FirmardocumentoNCRequest();
		DTE dte = new DTE();
	...
	public DTE construirDTE(Object entity, Boolean contingencia) {
		...
		ArrayList<DocumentoRelacionado> listaDocsRelacionados = new ArrayList<DocumentoRelacionado>();
		...
		try {
			...
			encabezadoDTE = dteJDBC.getEncabezadoNC(felControl.getuDocEntry());

			String numeroControl = "DTE-" + properties.getProperty("DTE_NC") + "-" + "B001P001" + "-"
			...
			identificacionDTE.setVersion(Integer.valueOf(properties.getProperty("VERSION_NC")));
			identificacionDTE.setAmbiente(properties.getProperty("AMBIENTE"));
			identificacionDTE.setTipoDte(properties.getProperty("DTE_NC"));
			...
			listaDetallesDTE = dteJDBC.getDetalleNC(felControl.getuDocEntry());
			listaDocsRelacionados = dteJDBC.getDocumentosRelacionados(felControl.getuDocEntry());
...
//			ventaTerceroDTE.setNombre(null);
			...
			for (DetalleDTE detalleDTE : listaDetallesDTE) {
				...
				cuerpoDocumentoDTE.setNumeroDocumento(listaDocsRelacionados.get(0).getNumeroDocumento());
			...
			dte.setDocumentoRelacionado(listaDocsRelacionados);
	...
	public RecepcionDTERequest construirRecepcionDte(String documentoFirmado) {
		...
		recepcionDTERequest.setVersion(Integer.valueOf(properties.getProperty("VERSION_NC")));
		recepcionDTERequest.setTipoDte(properties.getProperty("DTE_NC"));
