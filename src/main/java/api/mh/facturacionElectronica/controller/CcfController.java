...
import api.mh.facturacionElectronica.ex.model.ccf.ExtensionDTE;
...
public class CcfController extends AbstractController {
	...
	public DTE construirDTE(Object entity, Boolean contingencia) {
		...
		try {
			...
			String numeroControl = "DTE-" + properties.getProperty("DTE_CCF") + "-" + "B001P001" + "-"
			...
			identificacionDTE.setVersion(Integer.valueOf(properties.getProperty("VERSION_CCF")));
			identificacionDTE.setAmbiente(properties.getProperty("AMBIENTE"));
			identificacionDTE.setTipoDte(properties.getProperty("DTE_CCF"));
...
//			BigDecimal iva = resumenDTE.getSubTotal().multiply(BigDecimal.valueOf(0.13));
//			tributosDTE.setValor(iva);
	...
	public RecepcionDTEResponse procesarDTE(Object entity, Object token, Boolean contingencia) {
		...
		try {
			...
			firmardocumentoCCFRequest = obtenerDTE(felControl, contingencia);
			String jsonEntrada = gson.toJson(firmardocumentoCCFRequest);
			...
			jsonEntrada = gson.toJson(firmardocumentoCCFRequest.getDteJson());

			guardarRespuestaMH(recepcionDTEResponse, felControl, firmardocumentoResponse.getBody(),
					firmardocumentoCCFRequest.getDteJson().getIdentificacion().getNumeroControl(), jsonEntrada,
			...
			try {
				if (recepcionDTEResponse.getEstado().equals("PROCESADO")) {
					...
					envioEmail = email.enviarComprobanteDTE(felControl, jsonEntrada,
							firmardocumentoCCFRequest.getDteJson().getReceptor().getCorreo());
				}
			} catch (Exception e) {
				envioEmail = "NO";
			}
	...
	@SuppressWarnings("unchecked")
	@Override
	public FirmardocumentoCCFRequest obtenerDTE(Object entity, Boolean contingencia) {
		FelControl felControl = (FelControl) entity;
		FirmardocumentoCCFRequest firmardocumentoCCFRequest = new FirmardocumentoCCFRequest();
		DTE dte = new DTE();
		...
		firmardocumentoCCFRequest.setNit(properties.getProperty("USER_AUTH"));
		firmardocumentoCCFRequest.setPasswordPri(properties.getProperty("PSW_AUTH"));
		firmardocumentoCCFRequest.setActivo(true);
		firmardocumentoCCFRequest.setDteJson(dte);

		return firmardocumentoCCFRequest;
	}
	...
	public RecepcionDTERequest construirRecepcionDte(String documentoFirmado) {
		...
		recepcionDTERequest.setVersion(Integer.valueOf(properties.getProperty("VERSION_CCF")));
		recepcionDTERequest.setTipoDte(properties.getProperty("DTE_CCF"));
